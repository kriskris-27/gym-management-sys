...
exports.batchCleanupStaleSessions = batchCleanupStaleSessions;
 ...
 */
async function scanMember(phone, now = new Date()) {
    console.log(`[Attendance Domain] Scanning member with phone: ${phone}`);
    return await prisma_optimized_1.prisma.$transaction(async (tx) => {
        ...
        if (!todaySession) {
            ...
            try {
                ...
                if (error && typeof error === 'object' && 'code' in error && error.code === 'P2002') {
                    ...
                    if (existingSession) {
                        const duration = existingSession.checkOut ? (0, utils_1.calcDuration)(existingSession.checkIn, existingSession.checkOut) : 0;
                        return {
        ...
        if (todaySession.checkOut) {
            ...
            const duration = (0, utils_1.calcDuration)(todaySession.checkIn, todaySession.checkOut);
            return {
                state: "ALREADY_DONE",
                memberName: member.name,
                ...
                durationFormatted: (0, utils_1.formatDuration)(duration),
        ...
        return {
            state: "CHECKED_OUT",
            memberName: member.name,
            message: `Goodbye, ${member.name}! You stayed for ${(0, utils_1.formatDuration)(gap)} 💪`,
            checkInAt: todaySession.checkIn.toISOString(), // Use checkIn field
            checkOutAt: now.toISOString(),
            durationMinutes: gap,
            durationFormatted: (0, utils_1.formatDuration)(gap),
            sessionId: todaySession.id
        };
    });
}
...
async function cleanupMemberSessionsInTransaction(memberId, now, tx) {
    ...
    // Apply updates
    if (updates.length > 0) {
        await Promise.all(updates.map(update => tx.attendanceSession.update({
            where: { id: update.id },
            data: {
 ...
 */
function validateSession(sessionDay, checkIn, checkOut, now = new Date()) {
    const errors = [];
    const warnings = [];
    ...
    if (checkOut) {
        const duration = (0, utils_1.calcDuration)(checkIn, checkOut);
    ...
    if (!checkOut) {
        const durationSinceCheckIn = (0, utils_1.calcDuration)(checkIn, now);
 ...
 */
async function getMemberAttendanceHistory(memberId, startDate, endDate, includeAutoClosed = false) {
    ...
    if (startDate || endDate) {
        where.sessionDay = {};
        if (startDate)
            where.sessionDay.gte = startDate;
        if (endDate)
            where.sessionDay.lte = endDate;
    }
    // Filter out auto-closed sessions unless explicitly requested
    if (!includeAutoClosed) {
        where.autoClosed = false;
    }
    const sessions = await prisma_optimized_1.prisma.attendanceSession.findMany({
        where,
        include: {
            member: {
 ...
 * FIX: Proper valid session definition
 */
async function getAttendanceStats(memberId, startDate, endDate) {
    console.log(`[Attendance Domain] Getting attendance stats`);
    // Get valid sessions (FIX: correct definition)
    const validWhere = {};
    if (memberId)
        validWhere.memberId = memberId;
    if (startDate || endDate) {
        validWhere.sessionDay = {};
        if (startDate)
            validWhere.sessionDay.gte = startDate;
        if (endDate)
            validWhere.sessionDay.lte = endDate;
    }
    const validSessions = await prisma_optimized_1.prisma.attendanceSession.findMany({
        where: validWhere
    });
    // Filter for valid sessions
    const filteredValidSessions = validSessions.filter((session) => session.checkOut !== null && !session.autoClosed);
    ...
    if (startDate || endDate) {
        ...
        if (startDate)
            invalidWhere.sessionDay.gte = startDate;
        if (endDate)
            invalidWhere.sessionDay.lte = endDate;
    }
    const invalidSessions = await prisma_optimized_1.prisma.attendanceSession.count({
        where: invalidWhere
    });
    // Calculate statistics
    const totalSessions = filteredValidSessions.length;
    const totalMinutes = filteredValidSessions.reduce((sum, session) => {
        const duration = session.checkOut ? (0, utils_1.calcDuration)(session.checkIn, session.checkOut) : 0;
        return sum + duration;
    }, 0);
 ...
 */
async function batchCleanupStaleSessions(now = new Date()) {
    console.log(`[Attendance Domain] Batch cleanup of stale sessions`);
    const { startOfTodayIST } = (0, utils_1.getISTDateRange)();
    const CLEANUP_BATCH_SIZE = 20;
    // Find limited batch of sessions needing cleanup
    const sessionsToClean = await prisma_optimized_1.prisma.attendanceSession.findMany({
        where: {
            checkOut: null, // Use checkOut field
            OR: [
    ...
    // Apply updates
    if (updates.length > 0) {
        await Promise.all(updates.map(update => prisma_optimized_1.prisma.attendanceSession.update({
            where: { id: update.id },
            data: {
                checkOut: update.checkOut, // Use checkOut field
                autoClosed: update.autoClosed,
