...
exports.createSubscriptionWithDate = createSubscriptionWithDate;
exports.renewSubscription = renewSubscription;
exports.getMemberSubscriptionHistory = getMemberSubscriptionHistory;
exports.getSubscriptionExpiry = getSubscriptionExpiry;
exports.updateSubscriptionStatus = updateSubscriptionStatus;
exports.getGracePeriodDays = getGracePeriodDays;
 ...
 */
async function getActiveSubscription(memberId) {
    console.log(`[Subscription Domain] Getting active subscription for member: ${memberId}`);
    const subscription = await prisma_optimized_1.prisma.subscription.findFirst({
        where: {
            memberId,
            status: 'ACTIVE',
 ...
 */
async function createSubscription(memberId, planId, customPrice) {
 ...
 */
async function createSubscriptionWithDate(memberId, planId, customPrice, forceStartDate) {
    console.log(`[Subscription Domain] Creating subscription for member: ${memberId}, plan: ${planId}`);
    // Get plan details
    const plan = await prisma_optimized_1.prisma.plan.findUnique({
    ...
    const subscription = await prisma_optimized_1.prisma.subscription.create({
        data: {
            memberId,
            planId,
            startDate,
 ...
 */
async function renewSubscription(memberId, newPlanId, customPrice) {
    ...
    if (currentSubscription) {
        ...
        await prisma_optimized_1.prisma.subscription.update({
 ...
 */
async function getMemberSubscriptionHistory(memberId) {
    console.log(`[Subscription Domain] Getting subscription history for member: ${memberId}`);
    const subscriptions = await prisma_optimized_1.prisma.subscription.findMany({
        where: { memberId },
        include: { plan: true },
 ...
 */
async function getSubscriptionExpiry(memberId, graceDays = 3 // Configurable via Setting
) {
...
async function getGracePeriodDays() {
    try {
        const setting = await prisma_optimized_1.prisma.setting.findUnique({
            where: { key: 'grace_period_days' }
        });
        return setting?.value || 3; // Default to 3 days
    }
    catch {
        return 3; // Fallback
    }
